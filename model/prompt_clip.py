...
class CLIPAttentionPA(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.np = self.config.num_prompts
    ...
    def forward(
        ...
        # get query proj
        query_states = self.q_proj(hidden_states) * self.scale
        key_states = self._shape(torch.cat([
            self.k_proj(hidden_states[:, :-self.np, :]),
            self.prompt_k_proj(hidden_states[:, -self.np:, :]),
        ], dim=1), -1, bsz)
        # key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
        value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
...
class CLIPEncoderLayerPT(nn.Module):
    def __init__(self, config: CLIPConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = CLIPAttentionPA(config)
...
class PromptCLIP(nn.Module):
    def __init__(self, config: Config):
        ...
        for l in self.clip.vision_model.encoder.layers:
            l.self_attn.prompt_k_proj.weight.data.copy_(l.self_attn.k_proj.weight.data)
            l.self_attn.prompt_k_proj.bias.data.copy_(l.self_attn.k_proj.bias.data)
