...
class CLIPEncoderPSA(nn.Module):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        Ls = (config.image_size // config.patch_size) ** 2
        Lt = Ls + Nf * Np
        attention_mask = torch.empty(1, Nf, 1, Lt, Lt)
        attention_mask.fill_(float("-inf"))
        attention_mask[0, :, 0, :Ls, :] = 0
        for i in range(Nf):
            attention_mask[0, i, 0, Ls + i * Np : Ls + (i + 1) * Np, :Ls] = 0
            attention_mask[0, i, 0, Ls + i * Np : Ls + (i + 1) * Np, Ls + i * Np : Ls + (i + 1) * Np] = 0
...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.encoder = CLIPEncoderPSA(config)
